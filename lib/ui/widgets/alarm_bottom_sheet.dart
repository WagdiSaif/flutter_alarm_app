...
class _AlarmBottomSheet extends ConsumerState<AlarmBottomSheet> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (widget.alarm.repeatDays.isEmpty &&
          isAfterDaysFromToday(widget.alarm.nextTrigger)) {
                                    ...
                                    Row(
                                      ...
                                      children: [
                                        Text(
                                          formatTime12Hours(
                                            timeAlarmUpdate,
                                            use24Hour: MediaQuery.of(context)
                                        ...
                                        Row(
                                          children: [
                                            IconButton(
                                              onPressed: () async {
                                                final firstDate =
                                                    customDateTime(day: 1);

                                                final lastDate = customDateTime(
  ...
  Future<void> _saveChanges(
    ...
    final fireAt = customAlarmDate != null
        ...
        : calculateNextTriggerTime(timeAlarm);

    if (!context.mounted) return;

    final alarmTitle =
        '${getAlarmDay(fireAt.weekday).shortName}  ${formatTime12Hours(timeAlarm, use24Hour: MediaQuery.of(context).alwaysUse24HourFormat)} ${timeAlarm.period.name.toUpperCase()}. Swip to Stop';
