...
final alarmProvider = StreamProvider<List<AlarmModel>>((ref) {
  final alarms = ref.watch(alarmControllerProvider);

  return alarms.alarmStream;
});

final permissionProvider = Provider<PermissoinDialog>(
  (ref) => PermissoinDialog(),
);
...
class _AddAlarmScreen extends ConsumerState<AddAlarmScreen> {
  ...
  String _checkNextTriggerDay(AlarmModel alarm) {
    if (alarm.isEnabled && alarm.alarmDaysModel.isEmpty) {
      if (isAfterDaysFromToday(alarm.nextTrigger, day: 1)) {
        return formatDateMD(alarm.nextTrigger).toString();
      }
      if (isAfterDaysFromToday(alarm.nextTrigger, day: 0)) {
        return 'Tomorrow';
      }
  ...
  Future<void> pickTimeAndCreateAlarm(
    ...
    if (selectedTime != null) {
      final fireAt = calculateNextTriggerTime(selectedTime);
      if (!context.mounted) return;
      final alarmNote =
          '${getAlarmDay(fireAt.weekday).shortName}  ${formatTime12Hours(selectedTime, use24Hour: MediaQuery.of(context).alwaysUse24HourFormat)} ${selectedTime.period.name.toUpperCase()}. Click to Stop';
  ...
  Future<bool> _requestPlatformPermissions(BuildContext context) async {
    ...
    final isConfirmed = await ref
        ...
        .showPermissionGuideline(context);
  ...
  Future<bool> _requestIosPermissions(BuildContext context) async {
    ...
    if (status.isPermanentlyDenied) {
      ...
      if (isConfirmed != true || !context.mounted) return false;

      await ref.read(permissionProvider).showOpenSettingsPermission(context);
