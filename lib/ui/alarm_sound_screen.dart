...
class _AlarmSoundScreen extends ConsumerState<AlarmSoundScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      await ref.read(alarmSoundPlayerProvider.notifier).initAudioSession();
  ...
  Future<void> _loadSoundFromDevice() async {
    try {
      final pickFile = await FilePicker.pickFile(type: FileType.audio);
      if (pickFile != null) {
        final filePth = pickFile;
          ...
          IconButton(
            ...
            onPressed: () async {
              ...
              if (isPlayingSound) {
                ...
                if (soundId != null) {
                  await player.playSound(alarmSound.value, isAssetPath: false);
                } else {
                  await player.playSound(alarmSound.value, isAssetPath: true);
                }
              }
            },
          ),
